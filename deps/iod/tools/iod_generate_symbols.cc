...
int main(int argc, char *argv[]) {
    ...
    std::set<std::string> keywords = {"alignas", "alignof", "and", "and_eq", "asm", "auto", "bitand", "bitor", "bool",
    ...
    for (int i = 1; i < argc - 1; i++) {
        ...
        if (parse_file(argv[i]) < 0) {
            std::cerr << "error: passing file " << argv[i] << " failed" << std::endl;
            return EXIT_FAILURE;
        }
        std::cout << "file " << argv[i] << " successfully passed" << std::endl;
    }
