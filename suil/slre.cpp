...
namespace suil {
    namespace regex {
        ...
        static int match_op(const unsigned char *re, const unsigned char *s, regex_info *info) {
            int result = 0;
            switch (*re) {
                case '\\':
                    /* Metacharacters */
                ...
                default:
                    if (info->flags & suil::regex::ignore_case) {
        ...
        static int match_set(const char *re, int re_len, const char *s,
            ...
            while (len <= re_len && re[len] != ']' && result <= 0) {
                ...
                if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
                    re[len + 2] != '\0') {
                    result = info->flags & suil::regex::ignore_case ?
        ...
        static int doh(const char *s, int s_len, regex_info *info, int bi);

        static int bar(const char *re, int re_len, const char *s, int s_len,
                       regex_info *info, int bi) {
...
/* Process branch points */
        ...
        static int baz(const char *s, int s_len, regex_info *info) {
        ...
        static void setup_branch_points(regex_info *info) {
            int i, j;
            struct branch tmp;
        ...
        static int foo(const char *re, int re_len, const char *s, int s_len, regex_info *info) {
            int i, step, depth = 0;
            ...
            for (i = 0; i < re_len; i += step) {
                ...
                if (re[i] == '|') {
                    ...
                    info->branches[info->num_branches].bracket_index =
                            (int)(info->brackets[info->num_brackets - 1].len == -1 ?
                    ...
                    size_t ind = info->brackets[info->num_brackets - 1].len == -1 ?
                              info->num_brackets - 1 : depth;
        ...
        int match(const char *regexp, const char *s, size_t size, capture *caps, size_t ncaps, int flags) {
            regex_info info;
            ...
            info.caps     = caps;
            DBG(("========================> [%s] [%.*s]\n", regexp, s_len, s));
            return foo(regexp, (int) strlen(regexp), s, size, &info);
        }

        int match(const char *regexp, const char *buf, size_t size, std::vector<capture>& caps, int flags) {
            capture tmp[32];
            int status = regex::match(regexp, buf, size, tmp, 32, flags);
            if (status > 0) {
                for (int i = 0; i < 32; i++) {
                    caps.emplace_back(tmp[i]);
                }
            }
            return status;
        }
