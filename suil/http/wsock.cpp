...
namespace suil {
    namespace http {
        ...
        status_t websock::handshake(const request &req, response &res, websock_api &api) {
            SHA_CTX         sctx;
            ...
            res.end([&api](request &rq, response &rs) {
                // clear the request to free resources
                rq.clear();

                // Create a web socket
                websock ws(rq.adator(), api);
        ...
        void websock::handle() {
            ...
            zcstring key = std::move(zcstring(sock.id()).copy());
        ...
        coroutine void websock::broadcast(websock& ws, websock_api &api, void *data, size_t size) {
            ...
            api.broadcast(false, &ws, data, size);
        ...
        void websock_api::broadcast(bool local, websock* src,const void *data, size_t size) {
            strace("WebSockApi::broadcast local %d, src %p, data %p, size %lu",
                   local, src, data, size);

            async_t<int> async(-1);
            uint32_t wait = 0, ipc_wait = 0;
            if (!local) {
                ipc_wait = worker::broadcast(async, IPC_WSOCK_BCAST, data, size);
            }
            ...
            strace("waiting for %u-local, %u-ipc broadcasts to complete %ld",
                   wait, ipc_wait, mnow());
        ...
        void websock_api::notify_conn(bool conn) {
            wsock_conn_msg msg;
            msg.api_id = id;
            msg.conn   = (uint8_t) (conn? 1 : 0);
            worker::broadcast(IPC_WSOCK_CONN, &msg, sizeof(wsock_conn_msg));
        }

        void websock_api::setup_ipc() {
            // intialize websocket ipc
            assert(started);

            worker::ipcreg(IPC_WSOCK_CONN,
            [&](uint8_t /*src*/, const void *data, size_t/*len*/) {
                strace("received web socket connection event");

                wsock_conn_msg *msg = (wsock_conn_msg *) data;
            ...
            [&](uint8_t /*src*/, const void *data, size_t len) {
                strace("received web socket broadcast event - data %p, len %lu", data, len);
