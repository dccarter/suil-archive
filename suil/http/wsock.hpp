...
namespace suil {
    namespace http {
        ...
        private:
            ...
            void broadcast(bool local, websock* src, const void *data, size_t size);
        ...
        protected:
            ...
            static status_t handshake(const request&, response&, websock_api&);

            websock(sock_adaptor& adaptor, websock_api& api)
                : sock(adaptor),
                  api(api)
            {}
        ...
        inline void ws_handshake(const request& req, response& res, websock_api& api) {
            status_t  s = websock::handshake(req, res, api);
            res.end(s);
        }
