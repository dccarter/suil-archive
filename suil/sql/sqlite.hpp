...
namespace suil {
    namespace sql {
        ...
        struct sqlite_connection : LOGGER(dtag(SQLITE_CONN)) {
            ...
            template <typename __T>
            inline const char* type_to_string(const __T&, std::enable_if<std::is_integral<__T>::value>* = 0)
            { return "INTEGER"; }

            template <typename __T>
            inline const char* type_to_string(const __T&, std::enable_if<std::is_floating_point<__T>::value>* = 0)
            { return "REAL"; }

            inline const char* type_to_string(const std::string&)
            { return "TEXT"; }
        ...
        struct sqlite_db : LOGGER(dtag(SQLITE_DB)) {
            ...
            void init(const char *db, __O... opts) {
                ...
                if (path != nullptr) {
                    ...
                    if (options.has(sym(SYNCHRONOUS))) {
                        buffer_t qb(32);
                        qb << "PRAGMA synchronous = " << options.get(sym(SYNCHRONOUS), 2);
                        conn(qb);
                    }
