...
#define SPID_PARENT 0
    ...
    static void ipc_init(__worker&  wrk) {
        ...
        char name[16];
        snprintf(name, sizeof(name)-1, "suil/%hhu", spid);
        ...
        if (spid != SPID_PARENT) {
            ...
            lnotice(WLOG, "scheduled suil/%hhu on cpu %hhu", spid, wrk.cpu);
        }
    ...
    int worker::spawn(uint8_t n, work_t work, post_spawn_t ps, post_spawn_t pps) {
        ...
        if (n == 0 || spid != SPID_PARENT) {
            lnotice(WLOG, "worker suil/%hhu started", spid);
            ...
            ipc_init(wrk);
            lnotice(WLOG, "starting parent - %d", wrk.pid);
            ...
            lnotice(WLOG, "parent exiting - %d", sig_recv);
            __started = false;
        }
    ...
    coroutine void async_broadcast(uint8_t msg, void *data, size_t len) {
        ...
        ltrace(WLOG, "messages sent, free data copy %p, %lu",  data, mnow());
        // free the copy
        memory::free(data);
    }
    ...
    void worker::broadcast(uint8_t msg, const void *data, size_t len) {
        ...
        ltrace(WLOG, "worker::broadcast copy %p msg %02X, data %p len %lu",
               copy, msg, data, len);
    ...
    int worker_wait(bool quit) {
        ...
        for (uint8_t w = 1; w <= ipc->nworkers; w++) {
            ...
            lnotice(WLOG, "suil/%hhu exiting, status %d", wrk.id, status);
            wrk.active = false;
            break;
        }
